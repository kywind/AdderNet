...
class Vgg16Conv_bn(nn.Module):
    ...
    def __init__(self, num_cls=1000):
        ...
        Input
            number of class, default is 1k.
        """
        super(Vgg16Conv_bn, self).__init__()
        ...
        # index of conv
        self.conv_layer_indices = [0, 3, 7, 10, 14, 17, 20, 24, 27, 30, 34, 37, 40]
        # feature maps
        self.feature_maps = OrderedDict()
        # switch
        self.pool_locs = OrderedDict()
    ...
    def init_weights(self):
        ...
        for idx, layer in enumerate(vgg16_pretrained.features):
            ...
            if isinstance(layer, nn.BatchNorm2d):
                self.features[idx].weight.data = layer.weight.data
                self.features[idx].bias.data = layer.bias.data
                self.features[idx].running_mean.data = layer.running_mean.data
                self.features[idx].running_var.data = layer.running_var.data
                self.features[idx].eps = layer.eps
        # fine-tune Linear
        for idx, layer in enumerate(vgg16_pretrained.classifier):
            if isinstance(layer, nn.Linear):
                self.classifier[idx].weight.data = layer.weight.data
