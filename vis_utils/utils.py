...
/deep-learning-models/image-models/imagenet_class_index.json'
    ...
    if not os.path.exists('./vis_data/imagenet_class_index.json'):
        r = requests.get(class_index_path).content
        with open('./vis_data/imagenet_class_index.json', 'w+') as f:
            f.write(r.content)
    with open('./vis_data/imagenet_class_index.json') as f:
